...
const BoardContent = ({ board }) => {
  ...
  const handleDragOver = (event) => {
    ...
    */
    if (activeColumn._id !== overColumn._id) {
      moveCardBetweenDefferentColumns(overColumn, overCardId, active, over, activeColumn, activeDraggingCardId, activeDraggingCardData)
    }
  ...
  const conllisionDetetionStratery = useCallback((args) => {
    // trường hợp kéo column thì dùng thuật toán closestCorners
    if (acitveDragItemType === ACTIVE_DRAG_ITEM_TYPE.COLUMN) {
      return closestCorners({ ...args })
    }

    // Tìm các điểm giao nhau, va chạm - intersetion với con trỏ
    ...
    if (overId) {
      ...
      // phát hiện va chạm closestCenter hoặc closestCorners đều được. Tuy nhiên ở đây dùng closestCenter mượt hơn
